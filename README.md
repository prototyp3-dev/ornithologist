...
The ornithologist DApp is an integration between IoT devices and a Cartesi DApp. The idea is that users will do birdwatches as amateurs ornithologists, and will encounter birds which will be minted as NFTs. The birds will be as rare as their extinction risk, and longer and further the birdwalks more probability to encounter a rarer bird. 

The data will flow from the IoT devices to the Cartesi DApp. The Iot device sends position inputs to w3bstream servers. Then the users will do a web3 contract call to claim the birdwalk encountered bird. The w3bstream servers will detect the contract call and return a summary of the birdwalk to the contract, which will forward the result to the Cartesi DApp. The Cartesi Dapp will take the birdwalk summary and calculate probabilities to encounter the bird species on the same region of the birdwalk. Then Cartesi Dapp will do a number of random choices to get the bird species encountered depending on the time span of the birdwalk, and will save the rarer encountered bird on the user's account. 

Users will be able to duel other users comparing the traits
...
- users duel other users comparing bird traits
...
As final notes, the security of the IoT devices and the security of the W3bstream servers is out of the scope of this project.
...
To build the w3bstream wasm file youl will need go installed on your system and tinygo. Then
...
2. get the operator address on <project> -> settings
...
DATA_PATH="../data" \
EEA_BIRDS_FILE="Article12_2020_birdsEUpopulation.csv" \
AVONET_BIRDS_FILE="AVONET1_BirdLife.csv" \
python3 prepare-data.py
```

```shell
DAPP_BIRDS_GEO_FILE="../data/ART12_3035_distribution_data_without_sensitive.gpkg" \
DAPP_BIRDS_FILE="birds_data.csv" \
...
Send some data to the w3bstream server
...
The, on the solidity contract (solidity/contracts/Bird.sol) in Remix run:
...
The duels are performed in commit reveal approach to to compare birds' traits. The duel score will be persist  in on the birds information and the user in DApp account.

The flow of the duel is as follows:
1. an user can choose to duel any opponent. The message must contain the chosen trait to compare and the commitment of the bird chosen. which is the SHA512-256 of  'BIRD-NONCE'. The hash can be obtained in online tools such as [SHA512_256](https://emn178.github.io/online-tools/sha512_256.html). 
   1.a before the opponent sends his bird, the user can cancel the duel
2. the opponent sends a chosen bird
   2.a before the first user sends the reveal (and after the timeout period), the opponent can claim a timeout to win the duel
3. the first user sends the chosen bird with the nonce

Duel message examples:

```json
{"action":"duel","opponent":"0x7099...79c8","trait":"mass","commit":"1cf71...a5f9"}
...
{"action":"duel","opponent":"0x7099...79c8","bird":"9b253...c82a","nonce":"1...9"}
```

To withdraw the birds,the user can send the following message. If the bird was not minted yet it is minted first then tranfered to the user.
